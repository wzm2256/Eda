...
class MaDataset(torch.utils.data.Dataset):
	def __init__(
		...
		self.subset = subset
		# This is chosen so that the point cloud can be fitted into a unit cube.
		# When computing translation error, this factor needs to be divided.
		self.scale = 0.35


		if subset == 'test':
			if class_indices == 0:
				name_list_f = os.path.join(dataset_root, '3DL_record.pkl')
			elif class_indices == 1:
				name_list_f = os.path.join(dataset_root, '3DM_record.pkl')
			elif class_indices == 2:
				name_list_f = os.path.join(dataset_root, 'zero_test.pkl')
			else:
				raise NotImplementedError
		elif subset == 'train':
			if class_indices in [0, 1]:
				name_list_f = os.path.join(dataset_root, 'train_record.pkl')
			elif class_indices == 2:
				name_list_f = os.path.join(dataset_root, 'zero_train.pkl')
			else:
				raise NotImplementedError
		else:
			raise NotImplementedError
		...
		data_path = os.path.join(dataset_root, f'{subset}.pk')
	...
	def __getitem__(self, index):
		...
		current_scene = os.path.dirname(self.data_list[index][0])
		current_scene2 = os.path.dirname(self.data_list[index][1])

		current_scene_index = os.path.relpath(self.data_list[index][0], current_scene).split('_')[-1].split('.')[0]
		current_scene_index2 = os.path.relpath(self.data_list[index][1], current_scene2).split('_')[-1].split('.')[0]

		assert current_scene == current_scene2


		data_list  = [self.All_data_Dic[(current_scene, str(current_scene_index))][0],
		              self.All_data_Dic[(current_scene, str(current_scene_index2))][0]]

		normal_list = [self.All_data_Dic[(current_scene, str(current_scene_index))][1],
		               self.All_data_Dic[(current_scene, str(current_scene_index2))][1]]
...
# def dist_run_one_epoch(dataloader, sampler, epoch):
# 	sampler.set_epoch(epoch)
# 	for data in dataloader:
# 		# inputs, labels = data[0].to(device_id), data[1].to(device_id)
# 		PC, graph_list, bi_graph_list, index = data
# 		rank = dist.get_rank()
# 		# rank = 0
...
#
# 		# pdb.set_trace()
# 		print(f'rank: {rank}, epoch: {epoch}, index: {index}, piece: {list(set(PC.piece_index.cpu().detach().numpy()))}, '
# 		      f'batch: {list(set(PC.transform_batch.cpu().detach().numpy()))}')
#
#
# def simple_main(my_dataset):
#
...
# 	dataloader = DataLoader(my_dataset, batch_size=4, sampler=sampler, follow_batch=['x_cor', 'transform'], num_workers=2, drop_last=True)
#
# 	# dataloader = DataLoader(my_dataset, batch_size=4, follow_batch=['x_cor', 'transform'],
# 	#                         num_workers=2, drop_last=True)
# 	# sampler = None
#
#
# 	for epoch in range(10):
# 		dist_run_one_epoch(dataloader, sampler, epoch)
#

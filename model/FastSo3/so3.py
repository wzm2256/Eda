...
def init_edge_rot_mat(edge_vec_0, norm=None, eps=1e-5):
	if norm is None:
		edge_vec_0_distance = torch.norm(edge_vec_0, dim=1, p=2)
	else:
		edge_vec_0_distance = norm

	# Make sure the atoms are far enough apart,
	# remove due to compile issues
	# if torch.min(edge_vec_0_distance) < 1e-4:
	# 	logging.warning(f"Error edge_vec_0_distance: {torch.min(edge_vec_0_distance)}")

	norm_x = edge_vec_0 / ((edge_vec_0_distance.view(-1, 1)) + eps)

	edge_vec_2 = torch.randn(edge_vec_0.shape, device=edge_vec_0.device)
	edge_vec_2 = edge_vec_2 / (torch.norm(edge_vec_2, dim=1, p=2, keepdim=True) + eps)
	...
	# assert torch.max(vec_dot) < 0.99, f'The max inner product is {torch.max(vec_dot)}'
...
def wigner_D(lval, alpha, beta, gamma):
	...
	return _z_rot_mat_full(alpha, beta, gamma, J, lval)
	# return Xa @ J @ Xb @ J @ Xc

# @ torch.compile(dynamic=True, fullgraph=True, mode='max-autotune')
def _z_rot_mat_full(alpha:torch.Tensor, beta:torch.Tensor, gamma:torch.Tensor, J: torch.Tensor, lv:int):

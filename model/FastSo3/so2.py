...
class SO2_conv_e(torch.nn.Module):
    ...
    def __init__(self, c_in: int, c_out: int, c_hidden:int, c_hidden_0:int, L: int, c_L0_in=0, c_L0_out=0, bias: bool =True):
        '''
        ...
        self.c_hidden_0 = c_hidden_0
        self.bias = bias

        self.en0 = nn.Parameter(torch.empty((c_in * L + c_L0_in, c_hidden_0)))
        self.de0 = nn.Parameter(torch.empty((c_hidden_0, c_out * L + c_L0_out)))
        if bias:
            self.bias_0 = nn.Parameter(torch.zeros((1, c_L0_out)))
    ...
    def forward(self, x: torch.Tensor, x_L0: torch.Tensor, w: torch.Tensor, w_L0: torch.Tensor) -> \
        ...
        assert w_L0.shape[1:] == (self.c_hidden_0, ), 'Inconsistent w_0 shape'
        ...
        if self.bias:
            y_L0 = de_x0[:, :self.c_L0_out] + self.bias_0
        else:
        ...
        en_weight1 = torch.cat([torch.flip(self.en1, [3]), self.en1], -1) # c_in, L, c_hidden, 2* L
